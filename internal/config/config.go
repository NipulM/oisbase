...
func (p *ProjectConfig) AddService(serviceName string) {
	...
	if _, exists := p.Services[serviceName]; !exists {
		p.Services[serviceName] = Service{Instances: make(map[string]*Instance)}
	}
}
...
func (p *ProjectConfig) AddInstanceAccess(serviceType, instanceName, targetServiceType, targetInstanceName string, permissions []string) error {
...
func (p *ProjectConfig) AddInstanceRoute(serviceType, instanceName string, route Route) error {
	service, exists := p.Services[serviceType]
	if !exists {
		return fmt.Errorf("service type %s does not exist", serviceType)
	}
...
func (p *ProjectConfig) GetInstanceRoutes(serviceType, instanceName string) []Route {
	service, exists := p.Services[serviceType]
	if !exists {
		return nil
	}
